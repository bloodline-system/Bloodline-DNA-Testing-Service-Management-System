...
public class StaffServiceImpl implements StaffService {
    ...
    public CRUDorderResponse getOrderById(Long orderId) {
        ServiceOrder serviceOrder = orderServiceRepository.findById(orderId)
                .orElseThrow(() -> new ResourceNotFoundException("Service order not found for ID: " + orderId));
        CRUDorderResponse response = orderServiceMapper.toCRUDorderResponse(serviceOrder);
        if (response == null) {
            throw new ResourceNotFoundException("Failed to map service order to CRUD response");
        }
        return response;
    }
    ...
    @Override
    public Page<CRUDorderResponse> getOrdersPage(String username, String status, Pageable pageable) {
        List<CRUDorderResponse> all = (status != null && !status.isBlank())
                ? filterOrdersByStatus(status, username)
                : getForStaff(username);
    ...
    @Override
    public Page<TestResultsResponse> getTestResultsPage(String username, String status, Pageable pageable) {
        List<TestResultsResponse> all = (status != null && !status.isBlank())
                ? filterTestResultsByStatus(status, username)
