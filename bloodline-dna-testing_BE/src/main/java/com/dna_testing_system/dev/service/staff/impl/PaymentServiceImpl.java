...
public class PaymentServiceImpl implements PaymentService {
    ...
    @Override
    public Page<PaymentResponse> getPaymentsPage(String query, String status, Pageable pageable) {
        List<PaymentResponse> all = getPayments();

        if (query != null && !query.isBlank()) {
            String q = query.trim().toLowerCase();
            all = all.stream()
                    .filter(p ->
                        p.getId().toString().contains(q) ||
                        (p.getOrderId() != null && p.getOrderId().toString().contains(q)) ||
                        (p.getCustomerName() != null && p.getCustomerName().toLowerCase().contains(q)) ||
        ...
        if (status != null && !status.isBlank()) {
            try {
                PaymentStatus paymentStatus = PaymentStatus.valueOf(status.toUpperCase().replace("-", "_"));
                all = all.stream()
                        .filter(p -> p.getPaymentStatus() != null &&
                                p.getPaymentStatus().equalsIgnoreCase(paymentStatus.name()))
                        .collect(Collectors.toList());
        ...
        List<PaymentResponse> content = start > all.size() ? List.of() : all.subList(start, end);
