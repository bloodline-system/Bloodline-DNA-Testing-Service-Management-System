...
public class ManagerDashboardController extends BaseController {
    ...
    public ResponseEntity<Map<String, Object>> getDashboardApi() {
        try {
            ...
            List<ServiceOrderResponse> recentOrders = orderTaskManagementService.getServiceOrders().stream()
    ...
    @GetMapping("/api/v1/manager/services")
    @ResponseBody
    public ResponseEntity<List<MedicalServiceResponse>> getServicesApi() {
        return ResponseEntity.ok(medicalServiceManageService.getAllServices());
    }

    @GetMapping("/api/v1/manager/orders")
    @ResponseBody
    public ResponseEntity<List<ServiceOrderResponse>> getOrdersApi() {
        return ResponseEntity.ok(orderTaskManagementService.getServiceOrders());
    }
